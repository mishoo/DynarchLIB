...
(function(){
	...
	H.removeIndentation = function(code) {
		if (/^(\s+)/.test(code)) {
			var ind = new RegExp("^" + RegExp.$1, "gm");
			code = code.replace(ind, "");
		}
		return code;
	};
	...
	H.prototype = {
		...
		doItNow : function(code) {
			...
			if (!this.args.noTrim)
				code = code.replace(/\s+$/, "");
			var formatted = [], T = this.tokenParsers, m, unknown = "", tok, i, f = 0, tokens = this.tokens = [];
			while (code.length > 0) {
			...
			for (i = 0; i < formatted.length; ++i) {
				f = formatted[i];
				if (typeof f == "string")
					html(this.formatUnknown(f));
				else
					html(this.formatToken(f));
			}
			html = html.get();
	...
	H.registerLang = function(type, tokens) {
        ...
        H.cloneLang = function(type, newType) {
                return H.registerLang(newType, H.LANG[type].tokens.slice(0));
        };
