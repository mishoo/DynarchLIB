...
require 5;
package DynarchLIB::XML_TreeBuilder;
...
sub new {
    ...
    $self->{'_xml_parser'} = XML::Parser->new( 'Handlers' => {
        ...
        'Char' => sub {
            if ($in_cdata) {
                $cdata->attr(text => $cdata->attr('text') . $_[1]);
            } else {
        ...
        'CdataStart' => sub {
            (
                ...
                $cdata = $self->{'_element_class'}->new('~literal', 'text' => '<![CDATA[')
            );
            $in_cdata++;
        },
        ...
        'CdataEnd' => sub {
            $in_cdata--;
            $cdata->attr(text => $cdata->attr('text') . ']]>');
        },
